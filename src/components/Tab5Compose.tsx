...
export default function Tab5Compose() {
  ...
  const handleRunMatching = async () => {
    ...
    // 调用视觉API
    const result = await runVisionMatching(
      {
    ...
    if (result.success && result.color && result.iconIndex !== undefined && result.name) {
      ...
      // 自动触发合成，等待React挂载canvas后使用本次识别结果绘制
      setTimeout(() => {
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2 items-center">
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-gradient-to-r from-[#8B6F47] to-[#A67C52] hover:from-[#6F5839] hover:to-[#8B6F47] text-white text-xs font-bold rounded-lg transition-all cursor-pointer flex items-center gap-1.5 shadow-md disabled:opacity-50 disabled:cursor-not-allowed disabled:from-gray-400 disabled:to-gray-500"
                  title={!apiEndpoint || !apiKey || !selectedModel ? '请先配置API' : !screenshotBase64 ? '请先上传截图' : iconLibrary.length === 0 ? '请先添加icon' : '一键识别'}
                >
